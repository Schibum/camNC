 ...
 */
export function Kbd({ shortcut, className, ...props }: { shortcut: string } & React.HTMLAttributes<HTMLElement>) {
  ...
  return (
    <kbd
      ...
      {...props}>
      <span className="text-xs"> {modifiers.map(getModifierLabel).join(' ')}</span>
      {k.toUpperCase()}
    </kbd>
  );
}
