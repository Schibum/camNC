 ...
 */
export async function connect(options: ConnectOptions): Promise<{ stream: MediaStream; pc: RTCPeerConnection }> {
  const { share, pwd, tracker = 'wss://tracker.openwebtorrent.com/', onStatusUpdate } = options;
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.addEventListener('message', async ev => {
        try {
          ...
          const errorMessage = 'Error during WebSocket message processing: ' + (error instanceof Error ? error.message : String(error));
    ...
    const errorMessage = 'Error during connection setup: ' + (error instanceof Error ? error.message : String(error));
...
function setCodecPreferences(pc: RTCPeerConnection, preferredCodecMime?: string) {
  ...
  if (preferredCodecMime) {
    order = [preferredCodecMime, ...kPreferredCodecs.filter(c => c !== preferredCodecMime)];
  }
 ...
 */
async function getOffer(pc: RTCPeerConnection, timeoutMs: number = 5000, debounceMs = 1000): Promise<string> {
  try {
    ...
    if (pc.localDescription) {
      ...
      throw new Error('Failed to generate offer: No local description found after ICE process.');
    }
  } catch (err) {
 ...
 */
function waitForDebouncedIce(pc: RTCPeerConnection, debounceMs: number, overallTimeoutMs: number): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const handleIceCandidate = (e: RTCPeerConnectionIceEvent) => {
      ...
      if (!hasSeenSrflxOrRelay && e.candidate && typeof e.candidate.type === 'string' && ['srflx', 'relay'].includes(e.candidate.type)) {
        console.log('First srflx/relay candidate seen. Starting debounce mechanism.');
      ...
      if (hasSeenSrflxOrRelay && e.candidate) {
        ...
        console.log('Null candidate received (helper), end of candidates marker.');
      }
    };
