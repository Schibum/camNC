...
export async function connectGo2RtcWs(options: Go2RtcWsOptions): Promise<{
  ...
  const stream = new MediaStream(transceiver.receiver.track ? [transceiver.receiver.track] : []);
  ...
  const ws = new WebSocket(`${base.replace(/\/$/, '')}/api/ws?src=${encodeURIComponent(options.src)}`);
