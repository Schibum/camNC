...
async function init() {
  const cv = await (await import(/* @vite-ignore */ location.origin + '/opencv_js.js')).default();
  win.cv = cv;
...
// win.Module.onRuntimeInitialized = async function () {
//   win.cv = await win.cv;
//   initApp();
//   console.log('OpenCV.js is ready', win.cv);
// }
...
function SettingsOverlay({ onSelectSource, currentSource }: SettingsOverlayProps) {
  ...
  return (
    <>
      <button
        className="absolute top-4 right-4 z-50 bg-gray-800 bg-opacity-70 rounded-full p-3 text-white"
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2 max-h-[40vh] overflow-y-auto">
                  {videoDevices.map(device => (
                    ...
                    >
                      ...
                      {selectedDeviceId === device.deviceId && currentSource === 'webcam' && (
              ...
              <div className="pt-2">
                ...
                <div className={`w-full py-2 px-4 rounded ${currentSource === 'url' ? 'bg-blue-600' : 'bg-gray-600'} text-white`}>
                  ...
                  />
                  <button
                    className="mt-2 w-full py-1 px-3 bg-blue-500 text-white rounded"
...
function App() {
  ...
  useEffect(() => {
    ...
    const getWebcamStream = async () => {
      try {
        ...
        let selectedDevice = videoInputs[0]; // Default to first camera

        if (isMobile) {
          // Try to find a back-facing camera on mobile
  ...
  }, [isOpenCvReady]); // Only run when OpenCV is ready
  ...
  if (!isOpenCvReady) {
    return <div className="loading-message">Loading OpenCV.js...</div>;
  }
