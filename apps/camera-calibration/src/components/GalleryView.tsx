...
import { useTransition, animated, config as springConfig} from '@react-spring/web'; // Import config and SpringValue
    ...
    >
      ...
      <div className="gallery-item-overlay absolute inset-0 bg-black/50 flex justify-end items-start p-2 opacity-0 transition-opacity duration-200 ease-in-out group-hover:opacity-100 rounded-lg">
        <button
          ...
          className="delete-button bg-transparent text-white text-lg cursor-pointer p-1 leading-none"
...
export const GalleryView: React.FC<GalleryViewProps> = ({ onClose }) => {
  const { capturedFrames, deleteFrame, runCalibration, calibrationResult } = useCalibrationStore();
  ...
  const handleFrameDelete = (id: string, e?: React.MouseEvent) => { // Make event optional
    ...
    <div className="gallery-view absolute inset-0 bg-black/95 z-20 p-5 overflow-y-auto text-white flex flex-col">
          ...
          <div className="gallery-header flex justify-between items-center mb-5 flex-shrink-0">
            ...
            <div className="gallery-actions flex gap-4">
              ...
              <button
                ...
                className="calibrate-button bg-green-600 text-white rounded px-4 py-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Calibrate
              </button>
              <button onClick={onClose} className="close-button bg-transparent text-white text-2xl cursor-pointer">×</button>
            </div>
          </div>
      ...
      {transitions((style: CSSProperties, item: CapturedFrame | null) => {
        return item ? (
          ...
          <AnimatedDiv style={{ ...style, position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 30 }}>
            <FrameDetailView
              frame={item}
