...
const formSchema = z.object({
  connectionString: z.string().refine((val) => {
    if (!val.startsWith('webtorrent:?')) return false;
    try {
      const params = new URLSearchParams(val.substring('webtorrent:?'.length));
...
function parseConnectionString(connectionString: string): ParsedValues | null {
  try {
    ...
    const params = new URLSearchParams(connectionString.substring('webtorrent:?'.length));
...
export function ConnectForm({ onConnect }: { onConnect: (values: ParsedValues) => void }) {
  ...
  function onSubmit(values: FormValues) {
    ...
    if (parsed) {
      ...
      console.error("Invalid connection string passed submit validation:", values.connectionString);
...
function RouteComponent() {
    ...
    try {
      const stream = await connect({
        ...
        onStatusUpdate(update) {
          ...
          toast.info("Status", {
            description: update,
            duration: update === 'connected' ? 3000 : 600 * 1000,
            id: "status-toast",
          });
        },
      })
  ...
  return (<div className="relative min-h-screen overflow-hidden">
    ...
    >
      <video ref={videoRef} controls className="w-screen h-screen object-cover" />
    </div>
  </div>
  );
}
